...
tags:
  ...
  - Hilt
private: false
updated_at: '2026-04-06T10:44:04+09:00'
...
本記事では、Androidアプリ開発において登場する各種オブジェクト（`Application`、`Activity`、`ViewModel` など）のインスタンス生存期間とスコープについて整理する。各オブジェクトがどのタイミング・スコープで生成され、破棄されるのかを理解することで、状態管理やリソース管理の最適化、メモリリークの防止などにつながる。
