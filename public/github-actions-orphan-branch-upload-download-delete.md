...
通常のブランチとは異なり、履歴が完全に分離されているのが特徴です。そのため、GitHub に orphan ブランチをプッシュしていたとしても、後でブランチを削除すれば、その履歴ごとリポジトリから取り除かれます。これにより、不要になった生成物を確実に削除でき、リポジトリの肥大化を防ぐことができます。
...
これらと比べると、orphan ブランチは GitHub 上で完結し、ワークフローに依存せずにファイルを管理できます。また、保存期間に制限がない点も利点です。さらに、通常のブランチと同様に `raw.githubusercontent.com` 経由でファイルを参照できるため、プルリクエストの説明文に直接画像を埋め込むことも可能です。
...
ファイルの保存・取得・削除を行うたびに、個別に Git コマンドを扱うのは手間がかかります。そこで、orphan ブランチを用いたファイル管理を簡単に行うための GitHub Actions を作成しました。

詳しくは、それぞれの Actions の実装を参照してください。
