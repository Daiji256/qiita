...
従来の `ObservableObject` + `@StateObject` に代わる形で、iOS 17で `@Observable` + `@State` が登場し、よりモダンな状態管理が可能になりました。しかし、`@StateObject` が持っていた初期値の遅延評価やインスタンスは1度しか生成しないという特性を、`@State` プロパティラッパーは持っていません。
...
前節のサンプルコードをXcode 27でビルドすると、`Counter initialized` は初回描画時の1度しか実行されません。このことから、インスタンスの生成が1度であることがわかります。

このマクロ化によるパフォーマンス改善はコンパイラレベルでの変更であるため、Xcode 27を使ってビルドすれば、iOS 17などの過去のOSバージョンでも同様の恩恵を受けることができます。

## まとめ

- Xcode 26までは、親 `View` の再描画に伴い、子 `View` の `@State` で保持している `@Observable` インスタンスが無駄に生成と破棄をされていた
- Xcode 27からは `@State` がSwiftマクロ化され、初期値の遅延評価が行われるようになったため、この無駄な生成が完全に解消された
