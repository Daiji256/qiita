...
このとき、たとえ Kotlin 側の関数の実行が終了しても、Kotlin/Native 側で Swift オブジェクトへの参照をラップしたオブジェクトが生存している間は、Swift 側のインスタンスへの参照が残り続けます。そのラップオブジェクトが不要と判断されるタイミングは、Kotlin/Native の GC に依存します。

結果として、Swift 側では ARC の参照カウントが 0 にならず、`deinit` の呼び出しが Kotlin/Native の GC が走るタイミングまで遅れることになります。
