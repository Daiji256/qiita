---
title: Google Play Consoleでアプリ署名のフィンガープリントを確認する（2026年）
tags:
  - Android
  - fingerprint
  - keystore
  ...
  - GooglePlayConsole
private: false
updated_at: '2026-07-13T21:18:22+09:00'
...
しかし、現在のAndroidアプリ開発ではPlayアプリ署名（Play App Signing）を利用してGoogle Play側で署名鍵を管理することが主流となっており、手元にリリース用のキーストアを持たないことも多いでしょう。そのような場合、正確なフィンガープリントは[Google Play Console](https://play.google.com/console)上で確認する必要があります。

本記事では、2026年6月19日におけるPlay Console上でのフィンガープリント確認手順を紹介します。

## Play Console上でアプリ署名のフィンガープリントを確認する方法

最近のGoogle Play ConsoleのUI変更により、アプリ署名のフィンガープリント（SHA-1 / SHA-256）を確認する場所が変わりました。
