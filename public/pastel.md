...
本記事では、`ColorUtils.M3HCTToColor` / `ColorUtils.colorToM3HCT` を用いることで、ユーザー設定を活かした独自のカラースキームを作成できることを紹介します。詳細なテーマ設計ではなく、その可能性に焦点を当てます。
...
独自のカラースキームを作成するとき、それらを必ず担保する必要はありません。ただし、ユーザーのことを考えると、アクセシビリティやコントラスト設定は考慮したほうがよいです。
...
- Material Youが生成した色を参考に、独自のカラースキームを作成できる
- 自由なカラースキームの作成が可能な一方で、アクセシビリティやシステム設定（コントラスト等）は別途考慮が必要になる
