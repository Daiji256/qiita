...
通常、関数に渡すラムダ式の中で変数の初期化を試みても、コンパイラは「そのラムダ式が本当に 1 回だけ呼び出されるかわからない」ため、エラーとなります。しかし、`run` は「ラムダ式を必ず 1 回呼び出す：`callsInPlace(block, InvocationKind.EXACTLY_ONCE)`」とコンパイラに伝えているため、ラムダ内での初期化を認めます。
